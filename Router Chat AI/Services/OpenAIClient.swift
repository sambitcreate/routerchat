...
class OpenAIClient: AIClient {
    ...
    func sendMessage(_ text: String, model: String, history: [Message]) async throws -> String {
        ...
        return try await Task.detached(priority: .userInitiated) { () -> String in
            ...
            if !(200...299).contains(httpResponse.statusCode) {
                ...
                throw AIError.apiError("OpenAI API Error: HTTP \(httpResponse.statusCode)")
            }
    ...
    func streamMessage(_ text: String, model: String, history: [Message], onUpdate: @escaping (String) -> Void) async throws -> String {
        ...
        print("OpenAIClient: Sending request with model: \(model)")
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            if let errorData = errorData,
               let errorString = String(data: errorData, encoding: .utf8) {
                print("OpenAI Streaming Error - Status Code: \(httpResponse.statusCode)")
                print("Error Response: \(errorString)")
            ...
            throw AIError.streamingError("OpenAI API Error: HTTP \(httpResponse.statusCode)")
        }
